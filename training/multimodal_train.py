...
## Training dual-modal fusions.

# def train_multi_track_acc(model, architect, criterion, optimizer, scheduler, dataloaders, dataset_sizes, device, num_epochs, 
#                             parallel, logger, args, init_acc=0.0, status='_search'):
...
#                         audio, image, rf, label = data[0], data[1], data[2], data[3]
        
#                         if 'audio' in args.name:
#                             audio = audio.to(device).float()
#                         if 'visual' in args.name:
...
#                             if 'audio' in args.name and 'visual' in args.name:
#                                 _input = (audio, image)  # Correct order for audio and visual
#                             elif 'audio' in args.name and 'rf' in args.name:
#                                 _input = (audio, rf)  # Correct order for audio and RF
#                             elif 'visual' in args.name and 'rf' in args.name:
#                                 _input = (image, rf)  # Correct order for visual and RF
#                             architect.step(_input, label, logger)
...
#                         batch_acc = accuracy_score(batch_pred_th.numpy(), batch_true) * 100                  
...
def train_multi_track_acc(model, architect, criterion, optimizer, scheduler, dataloaders, dataset_sizes, device, num_epochs, 
    ...
    while failsafe:
        for epoch in range(num_epochs):
            ...
            phases = ['train', 'test'] if status != 'search' else ['train', 'dev']
            ...
            for phase in phases:
                ...
                with tqdm(dataloaders[phase]) as t:
                    for data in t:
                        audio, image, rf, label = data
                        audio, image, rf, label = audio.to(device).float(), image.to(device).float(), rf.to(device).float(), label.to(device).float()

                        if status == 'search' and (phase == 'dev' or phase == 'test'):
                            _input = (image, audio, rf)
                            architect.step(_input, label, logger)
...
## Testing dual-modal fusions.

# def test_multi_track_acc(model, criterion, dataloaders, device, args, phase='test', warmup_iters=5, cm_filename='confusion_matrix_multi.png'):
#     top1_test = []
#     top5_test = []                    
#     model.eval()  # Set model to evaluate mode
#     list_preds = [] 
...
#     # Calculate average prediction time, excluding warm-up iterations
...
## Testing tri-modal fusions.

def test_multi_track_acc(model, criterion, dataloaders, device, phase='test', warmup_iters=5, cm_filename='confusion_matrix_multi.png'):
    top1_test = []
    top5_test = []                    
    model.eval()  # Set model to evaluate mode
    list_preds = [] 
